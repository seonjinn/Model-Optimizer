...
POLICY="" OUTPUT_DIR="" ACCOUNT="" PARTITION="" WALLTIME="" DRY_RUN=0

usage() {
    echo "usage: $0 --source-inventory PATH --tokenizer-receipt PATH --baseline-audit PATH --held-out-uuids PATH --policy PATH --output-dir PATH --account NAME --partition NAME --time HH:MM:SS [--dry-run]" >&2
    exit 2
}
while (( $# )); do
    case "$1" in
...
for name in SOURCE_INVENTORY TOKENIZER_RECEIPT BASELINE_AUDIT HELD_OUT_UUIDS POLICY OUTPUT_DIR ACCOUNT PARTITION WALLTIME; do
    [[ -n "${!name}" ]] || usage
done
for path in "$SOURCE_INVENTORY" "$TOKENIZER_RECEIPT" "$BASELINE_AUDIT" \
    "$HELD_OUT_UUIDS" "$POLICY" "$OUTPUT_DIR"; do
    [[ "$path" == /* ]] || usage
done
