...
"""Build the three-arm Q30-Thinking plumbing canary manifest.
...
def build_canary_experiments(
    *,
    ...
    """Return one 20-step canary experiment per drafter architecture.
    ...
    """
    base = TargetTopology.for_kind(_TARGET_KIND)
    ...
    for method in CANARY_METHODS:
        run_name = f"{run_prefix}-{method}-b8-{slurm.nodes}n"
        experiments.append(
            DrafterExperiment(
                target=target_label,
