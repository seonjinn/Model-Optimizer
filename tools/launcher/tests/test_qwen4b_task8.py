...
def _response_artifact(
    ...
    *,
            ...
            _canonical(
                {
                    "ordinal": row[0],
                    "prompt_uuid": row[1],
                    "source_identity_sha256": row[2],
                    "source_row": row[3],
                    "conversation_sha256": row[4],
                    "assistant_response_sha256": row[5],
                }
            )
            + b"\n"
            for row in rows
    ...
    body = {
        ...
        "occurrence_count": view.occurrence_count if occurrence_count is None else occurrence_count,
        "files": [
            {
                "path": records.name,
...
def _rejection_artifact(
    root: Path,
    view: SimpleNamespace,
    records_payload: tuple[dict[str, object], ...] = (),
    *,
    source_commit: str = "a" * 40,
) -> tuple[Path, str]:
    root.mkdir()
    records = root / "rejection.jsonl"
    records.write_bytes(b"".join(_canonical(row) + b"\n" for row in records_payload))
    reason_counts: dict[str, int] = {}
    for row in records_payload:
        reason = str(row["rejection_reason"])
        reason_counts[reason] = reason_counts.get(reason, 0) + 1
    receipt = root / "rejection.json"
    body = {
        "schema_version": 1,
        "role": "rejection",
        "source_commit": source_commit,
        ...
        "rejection_count": len(records_payload),
...
def test_response_declared_file_is_authenticated_before_tokenization(tmp_path: Path) -> None:
    ...
    view, _ = _selection(tmp_path)
    response, response_sha = _response_artifact(tmp_path / "response", view)
    rejection, rejection_sha = _rejection_artifact(tmp_path / "rejection", view)
...
def test_response_occurrence_count_cannot_misrepresent_selection(tmp_path: Path) -> None:
    ...
    view, _ = _selection(tmp_path)
    response, response_sha = _response_artifact(tmp_path / "response", view, occurrence_count=1)
    rejection, rejection_sha = _rejection_artifact(tmp_path / "rejection", view)
...
def test_semantically_empty_response_file_is_rejected(tmp_path: Path) -> None:
    """A self-authenticated file still has to carry the exact selected response rows."""
    view, _ = _selection(tmp_path)
    ...
    rejection, rejection_sha = _rejection_artifact(tmp_path / "rejection", view)
    with pytest.raises(task8.Task8BuildError, match="response record schema"):
        task8.validate_required_role_receipts(
            view=view,
            task9_source_commit="a" * 40,
            response_receipt=response,
...
def test_semantically_empty_rejection_file_is_rejected(tmp_path: Path) -> None:
    """A nonzero rejection receipt must identify each quarantined occurrence."""
    view, _ = _selection(tmp_path)
    response, response_sha = _response_artifact(tmp_path / "response", view)
    rejection, rejection_sha = _artifact(
        ...
        "rejection",
        {
            ...
            "occurrence_count": view.occurrence_count,
            "rejection_count": 1,
            "reason_counts": {"invalid-assistant-mask": 1},
        },
    )
    with pytest.raises(task8.Task8BuildError, match="rejection record schema"):
        task8.validate_required_role_receipts(
            view=view,
            task9_source_commit="a" * 40,
            response_receipt=response,
...
def test_receipt_ancestor_symlink_is_rejected(tmp_path: Path) -> None:
    """Opening a receipt must not follow a symlink in any ancestor component."""
    view, _ = _selection(tmp_path)
    actual = tmp_path / "actual-response"
    response, response_sha = _response_artifact(actual, view)
    linked = tmp_path / "linked-response"
    linked.symlink_to(actual, target_is_directory=True)
    rejection, rejection_sha = _rejection_artifact(tmp_path / "rejection", view)
    with pytest.raises(task8.Task8BuildError, match=r"response receipt.*unsafe"):
        task8.validate_required_role_receipts(
            view=view,
            task9_source_commit="a" * 40,
            response_receipt=linked / response.name,
            response_receipt_sha256=response_sha,
            rejection_receipt=rejection,
            rejection_receipt_sha256=rejection_sha,
        )
...
def test_publication_root_rejects_traversal_and_symlink_ancestors(tmp_path: Path) -> None:
    """Durable-root containment is lexical and every existing ancestor is no-follow."""
    durable = tmp_path / "durable"
    durable.mkdir()
    safe_parent = durable / "task8"
    safe_parent.mkdir()
    assert task8.validate_durable_publication_root(safe_parent / "publication", durable) == (
        safe_parent / "publication"
    )
    with pytest.raises(task8.Task8BuildError, match="canonical"):
        task8.validate_durable_publication_root(f"{safe_parent}/../escape", durable)

    actual = tmp_path / "actual"
    actual.mkdir()
    (durable / "linked").symlink_to(actual, target_is_directory=True)
    with pytest.raises(task8.Task8BuildError, match="symlink"):
        task8.validate_durable_publication_root(durable / "linked" / "publication", durable)
...
def test_runner_rejects_publication_root_traversal_before_mkdir(tmp_path: Path) -> None:
    """A lexical descendant containing dot traversal is never normalized into scope."""
    runner = _REPO_ROOT / "tools/launcher/common/specdec/run_qwen4b_task8.sbatch"
    durable = (
        "/lustre/fs1/portfolios/coreai/projects/coreai_dlalgo_nemorl/users/sna/"
        "modelopt-specdec/dataset-studies"
    )
    environment = {
        "PATH": "/usr/bin:/bin",
        "SLURM_JOB_ID": "7",
        ...
        "IMAGE_PATH": str(tmp_path / "image.sqsh"),
        "IMAGE_SHA256": "8" * 64,
        "PUBLICATION_ROOT": f"{durable}/task8/../escape",
    }
    result = subprocess.run(
        ["bash", str(runner)], check=False, capture_output=True, text=True, env=environment
    )
    assert result.returncode == 2
...
def test_submitter_rejects_publication_root_traversal_before_mkdir(tmp_path: Path) -> None:
    """The host preflight must reject traversal before git, mkdir, or scheduler calls."""
    submitter = _REPO_ROOT / "tools/launcher/common/specdec/submit_qwen4b_task8.sh"
    inputs: list[str] = []
...
def test_submitter_dry_run_executes_only_scheduler_test_only(tmp_path: Path) -> None:
    ...
    assert "TASK9_SOURCE_COMMIT=" + "b" * 40 in submitted[0]
    assert f"--chdir={_REPO_ROOT}" in submitted[0]
